    ...
    <main className="min-h-screen p-8 space-y-6">
      <div className="flex items-center gap-4">
        {user.image ? (
          ...
          <img src={user.image} alt={`${user.name ?? user.handle} avatar`} className="w-12 h-12 rounded-full" />
        ) : null}
        <div>
      ...
      {p ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <section className="space-y-2">
            ...
            {p.poolroomAlias && <p><strong>Poolroom Alias:</strong> {p.poolroomAlias}</p>}
            {p.cue && <p><strong>Cue:</strong> {p.cue}</p>}
          ...
          <section className="space-y-2">
            ...
            <div className="flex gap-4">
              {p.cashapp && (
                <a className="underline" href={`https://cash.app/${p.cashapp.replace(/^\$/, "")}`} target="_blank">
                  Cash App
                </a>
              )}
              {p.venmo && (
                <a className="underline" href={`https://venmo.com/${p.venmo.replace(/^@/, "")}`} target="_blank">
                  Venmo
                </a>
              )}
            </div>
            {typeof p.views === "number" && <p className="text-sm text-gray-500">Views: {p.views}</p>}
          </section>
        </div>
      ) : (
